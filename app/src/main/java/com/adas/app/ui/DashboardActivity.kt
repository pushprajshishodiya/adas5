...
class DashboardActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.previewRear.setOnClickListener {
            binding.adasOverlay.let { o ->
                // tap PIP to toggle overlay mode
            }
        }
    ...
    private fun setupDrawer() {
        ...
        tvProfile.text = "${profile.name}\n${profile.make} ${profile.model} ${profile.year}\n" +
            "W:${profile.widthM}m  WB:${profile.wheelbaseM}m\nCam H:${profile.cameraHeightM}m  FOV:${profile.cameraFovDeg}°\n" +
            "Steer ratio: ${profile.steeringRatioFull}:1\nMin radius: ${profile.turningRadiusM}m"
        ...
        swFront.setOnCheckedChangeListener { _, on ->
            ...
            cameras.rebind(this, binding.previewFront, binding.previewRear)
        }
        ...
        // Live sensor info in drawer
        lifecycleScope.launch {
            vm.sensorFusion.data.collectLatest { s ->
                runOnUiThread {
                    tvHeading.text = "Heading: ${"%.1f".format(s.headingDeg)}° ${compassDir(s.headingDeg)}\n" +
                        "Pitch: ${"%.1f".format(s.pitchDeg)}°  Roll: ${"%.1f".format(s.rollDeg)}°"
                    tvAccel.text = "Accel X(lat): ${"%.2f".format(s.accelX)}m/s²\n" +
                        "Accel Y(lon): ${"%.2f".format(s.accelY)}m/s²\n" +
                        "GPS acc: ${"%.0f".format(s.gpsAccuracyM)}m"
                }
            }
        }
    }
    ...
    private fun setupCameras() {
        ...
        cameras.start(this, binding.previewFront, binding.previewRear)
    }
    ...
    private fun updateUi(f: AdasFrame) {
        ...
        if (fn != null) {
            ...
            binding.tvFrontLabel.text = "CLEAR"; binding.tvFrontDist.text = "—"
            binding.tvFrontSpeed.text = ""; binding.cardFront.setCardBackgroundColor(warnColor(WarningLevel.SAFE))
        }
        ...
        if (rn != null) {
            ...
            binding.tvRearLabel.text = "CLEAR"; binding.tvRearDist.text = "—"
            binding.tvRearSpeed.text = ""; binding.cardRear.setCardBackgroundColor(warnColor(WarningLevel.SAFE))
        }
        ...
        binding.tvSteeringSpeed.text  = "Max speed: ${sg.recommendedSpeedKmh.roundToInt()} km/h"

        // Lane offset bar
        val offsetFrac = ((lane.laneOffsetFraction + 1f) / 2f).coerceIn(0f, 1f)
        setBar(binding.laneOffsetBar, offsetFrac, barTrackH.takeIf{it>0}?:20, Color.parseColor("#00E5FF"))
        ...
        if (worst != null && worst != WarningLevel.SAFE) {
            val msg = f.frontWarning?.let { "▲ FRONT ${it.vehicle.label.uppercase()} ${"%.0f".format(it.vehicle.distanceM)}m" }
                ?: f.rearWarning?.let { "▼ REAR ${it.vehicle.label.uppercase()} closing ${"%.0f".format(it.vehicle.relativeSpeedKmh)}km/h" }
    ...
    override fun onDestroy() {
        super.onDestroy(); cameras.stop(); alerts.release()
    }
}
